...
The coefficient with income is that when the income is increased by one "bucket", if you hold all the other variables constant, then the prediction that an individual would vote democratic is decreased by 0.695. We are 95% confident the true change lies within the range of decreasing by -2.2251542 or adding by 	0.81469977. Works only around center of distribution, not causal, explanatory. 
...
Better result if we use like 40 trees. 

What is OOB error?

Our model has an error rate of 25.43%
...
library(d3rain)
nes %>%
  d3rain(income, race, toolTip = income, title = "Income Brackets vs. Race") %>%
...
ggplot(nes, aes(x = income, y = dvote)) + geom_jitter(alpha = 0.3, height = 0.4) + 
